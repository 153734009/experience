 ...
 * ===============================
 * log:
 *	201408231344	Uninitialized string offset的错误时，数组key值越界
 *		可以增加 isset($arr[key]) 的判断
 *		或者 选择忽视该提示，将php.ini中error_reporting = E_ALL 改为error_reporting = E_ALL & ~E_NOTICE即可
 *	
 *
 */
namespace Lib\segment;
...
class Segment {
	...
	function __construct($loaddic=true) {
  	if($loaddic) {
  	  ...
  	  $dicfile = dirname(__FILE__).DIRECTORY_SEPARATOR.'dict.utf8.csv';
  	  $fp = fopen($dicfile,'r');
