...
function fullZip($path="./",$zip){	
	...
	if ($handle) {
		...
		if($file != "." && $file != ".."){
			...
			$path_sub = str_replace("//", "/", $path_sub);
			$path_sub = str_replace("./", "", $path_sub);
			$path_sub = rtrim($path_sub, "/");
			if(is_dir($path_sub)){
				fullZip($path_sub,$zip);
			}else{
				if(substr($path_sub,-3,3)!='zip'){
				$zip->addFile(file_get_contents($path_sub),$path_sub, filectime($path_sub), NULL, TRUE, Zip::getFileExtAttr($file));
				}
			}
		}	
		}
